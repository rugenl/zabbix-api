...
items = zapi.item.get(
    hostid=hostId, applicationids=appid, output="extend", selectTriggers="extend"
)
...
for interface in sorted(host["interfaces"], key=itemgetter("main"), reverse=True):
    ...
    if interface["port"] != "80":
        ...
        this_key = (
            "cert_days_left.sh[" + interface["dns"] + "," + interface["port"] + "]"
        )
        item_resp = zapi.item.create(
            {
                "name": "$1 SSL Certificate days till expiration",
                "key_": this_key,
                "type": "10",
                "hostid": hostId,
                "value_type": "0",
        ...
        trigger_resp = zapi.trigger.create(
            {
                "description": interface["dns"] + " Certificate expires within 3 days",
        ...
        trigger_resp = zapi.trigger.create(
            {
                "description": interface["dns"] + " Certificate expires within 10 days",
        ...
        trigger_resp = zapi.trigger.create(
            {
                "description": interface["dns"] + " Certificate expires within 15 days",
        ...
        trigger_resp = zapi.trigger.create(
            {
                "description": interface["dns"] + " Certificate expires within 30 days",
    ...
    trigger_resp = zapi.trigger.create(
        {
            "description": interface["dns"] + " http web site problem",
            "expression": "{"
            + prettyHostName
            + ":web.test.fail["
            + interface["dns"]
    ...
    if interface["port"] != "80":
        ...
        trigger_resp = zapi.trigger.create(
            {
                "description": interface["dns"] + " https web site problem",
                "expression": "{"
                + prettyHostName
                + ":web.test.fail["
                + interface["dns"]
